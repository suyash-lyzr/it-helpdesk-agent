...
import { getKPIMetrics, getSLAFunnel, getTopIssues, getTeamPerformance } from "@/lib/analytics-store"
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (schedule) {
      ...
      appendAuditLog("system", "report_scheduled", `Scheduled report: ${scheduledReport.name}`)
    }
    ...
    // Return JSON report
    appendAuditLog("system", "report_generated", `Report generated with metrics: ${metrics.join(", ")}`)
